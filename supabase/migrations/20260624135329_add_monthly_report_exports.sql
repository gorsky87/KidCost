-- KidCost monthly report API and CSV export.
-- Scope: issue #30.

create or replace function public.monthly_report_csv_cell(value text)
returns text
language sql
immutable
as $$
...
declare
  month_start date;
  month_end date;
  csv_header text := 'data,dziecko,kategoria,opis,płacący,kwota,status';
  csv_rows text;
begin
  if target_family_id is null then
    raise exception 'target_family_id is required'
